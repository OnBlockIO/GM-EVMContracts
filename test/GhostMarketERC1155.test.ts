...
describe("GhostMarket ERC1155 Test", function () {
  ...
  beforeEach(async function () {
    ...
    erc1155_proxy = <GhostMarketERC1155>await upgrades.deployProxy(      
      ERC1155,
  ...
  it("should upgrade contract", async function () {
    ...
    //upgrade
    const ghostMarketERC1155_V2 = await upgrades.upgradeProxy(ghostMarketERC1155.address, GhostMarketERC1155_V2_ContractFactory);
    ...
    await expect(result)
      .to.emit(ghostMarketERC1155_V2, "NewMintFeeIncremented")
        .withArgs('100000000000000001')
  })
  ...
  describe('mintWithURI', function () {
    ...
    it("should revert if minter using mintWithURI function has not the POLYNETWORK_ROLE", async function () {
      ...
      await expect(testingAsSigner1.mintWithURI(addrs[0].address, tokenId, tokenId, MINT_AMOUNT, { from: addrs[1].address }))
  ...
  describe('burn NFT', function () {
    ...
    it('should revert if not-owner tries to burn a NFT', async function () {
      ...
      await expect(testingAsSigner1.burn(addrs[1].address, tokenId, MINT_AMOUNT, { from: addrs[1].address }))
    ...
    it('should revert if not-owner tries to burn a NFTs', async function () {
      ...
      await expect(testingAsSigner3.burnBatch(
        addrs[0].address,
        tokenBatchIds,
        burnAmounts,
  ...
  describe('mint with royalty', function () {
    it('should set royalties', async function () {
      ...
      await erc1155_proxy.mintGhost(addrs[1].address, MINT_AMOUNT, DATA, [{ recipient: addrs[2].address, value: royaltyValue }], "ext_uri", "", "");
      const tokenId = await getLastTokenID(erc1155_proxy)
    ...
    it('should mint tokens with royalty fee and address', async function () {
      ...
      const result = erc1155_proxy.mintGhost(addrs[1].address, MINT_AMOUNT, DATA, [{ recipient: addrs[0].address, value: value }], "ext_uri", "", "");
      const tokenId = await getLastTokenID(erc1155_proxy)
      await expect(result).to.emit(erc1155_proxy, "TransferSingle")
      ...
      await expect(result).to.emit(erc1155_proxy, "Minted")
        .withArgs(addrs[1].address, tokenId, "ext_uri", MINT_AMOUNT)
    });

    it('should revert if royalty is more then 50%', async function () {
      const value = 5001
      await expect(erc1155_proxy.mintGhost(addrs[1].address, MINT_AMOUNT, DATA, [{ recipient: addrs[0].address, value: value }], "ext_uri", "", ""))
  ...
  describe('mint NFT with fee', function () {
    ...
    it('should send fee to contract from another account then the contract owner', async function () {
      ...
      await testingAsSigner2.mintGhost(addrs[2].address, MINT_AMOUNT, DATA, [], "ext_uri", "", "ts", { value: value, from: addrs[2].address })
      const feeAddressEthBalanceAfter = await ethers.provider.getBalance(erc1155_proxy.address)
      expect(feeAddressEthBalanceAfter).to.equal(feeAddressEthBalanceBefore.add(value))
    });
  });
  ...
  describe('withdraw from contract', function () {
    it('should withdraw all available balance from contract', async function () {
      ...
      await testingAsSigner2.mintGhost(addrs[0].address, MINT_AMOUNT, DATA, [], "ext_uri", "", "ts", { value: value, from: addrs[2].address })
      await testingAsSigner2.mintGhost(addrs[0].address, MINT_AMOUNT, DATA, [], "ext_uri", "", "ts", { value: value, from: addrs[2].address })
      await testingAsSigner2.mintGhost(addrs[0].address, MINT_AMOUNT, DATA, [], "ext_uri", "", "ts", { value: value, from: addrs[2].address })
      const feeAddressEthBalanceAfter = await ethers.provider.getBalance(erc1155_proxy.address)
    ...
    it('should revert if trying to withdraw more then the contract balance', async function () {
      ...
      await erc1155_proxy.mintGhost(addrs[0].address, MINT_AMOUNT, DATA, [], "ext_uri", "", "ts", { value: value })
      const feeAddressEthBalanceAfter = await ethers.provider.getBalance(erc1155_proxy.address)
      await expect(erc1155_proxy.withdraw(feeAddressEthBalanceAfter.add(value)))
        .revertedWith("Withdraw amount should be greater then 0 and less then contract balance")
    });
    ...
    it('should revert if other then the contract owner tries to withdraw', async function () {
      ...
      await erc1155_proxy.mintGhost(addrs[0].address, MINT_AMOUNT, DATA, [], "ext_uri", "", "ts", { value: value })
      const feeAddressEthBalanceAfter = await ethers.provider.getBalance(erc1155_proxy.address)
      await expect(testingAsSigner2.withdraw(feeAddressEthBalanceAfter, { from: addrs[2].address }))
  ...
  describe('mint with locked content', function () {
    ...
    it("should set and get locked content for nft", async function () {
      ...
      await expect(result).to.emit(erc1155_proxy, "LockedContentViewed")
    ...
    it("should revert if other then token owner tries to fetch locked content", async function () {
      ...
      await expect(testingAsSigner3.getLockedContent(tokenId, { from: addrs[3].address }))
        .revertedWith("Caller must be the owner of the NFT")
    });
