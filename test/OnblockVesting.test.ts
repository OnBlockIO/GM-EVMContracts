...
describe("Onblock Vesting Test", function () {
  ...
  before('Deploy Contracts', async() => {
    ...
    dft_proxy = <DeflationaryToken>await upgrades.deployProxy(DFT,['DeflationaryToken', 'DFT', '10000000000000000', '8']);
  ...
  describe("Contract", function () {
    ...
    it("should fail, because is not voter", async () => {
        await expect(testingAsSigner5.setVaultFee(ethers.BigNumber.from('2000000000'), { from: addrs[5].address }))
    ...
    it("should fail without vote", async () => {
        await expect(testingAsSigner2.setVaultFee(ethers.BigNumber.from('9999'), { from: addrs[2].address }))
    ...
    it("should fail, because fee to high", async () => {
        await expect(testingAsSigner2.setVaultFee(ethers.BigNumber.from('9000000000000000000'), { from: addrs[2].address }))
    ...
    it("should create new fee update vote", async () => {
        const receipt = testingAsSigner1.requestVote(ethers.BigNumber.from('3'), ZERO_ADDRESS, ethers.BigNumber.from('20000000'), { from: addrs[1].address });
        await expect(receipt).to.emit(obv, 'VoteRequested')
    ...
    it("should vote on new fee update vote and finish", async () => {
      const createReceipt = testingAsSigner1.requestVote(ethers.BigNumber.from('3'), ZERO_ADDRESS, ethers.BigNumber.from('20000000'), { from: addrs[1].address });
      await expect(createReceipt).to.emit(obv, 'VoteRequested')
        .withArgs(addrs[1].address, ZERO_ADDRESS, ethers.BigNumber.from('20000000'), ethers.BigNumber.from('3'))

      const voteReceipt = testingAsSigner2.vote(ethers.BigNumber.from('3'), addrs[1].address, ZERO_ADDRESS, ethers.BigNumber.from('20000000'), { from: addrs[2].address });
      await expect(voteReceipt).to.emit(obv, 'Voted')
        .withArgs(addrs[2].address, ZERO_ADDRESS, ZERO_ADDRESS, ethers.BigNumber.from('20000000'), ethers.BigNumber.from('3'))

      const voteStateReceipt = testingAsSigner1.isVoteDone(ethers.BigNumber.from('3'), ZERO_ADDRESS, ethers.BigNumber.from('20000000'), { from: addrs[1].address });
      await expect(voteStateReceipt).to.emit(obv, 'VoteState')
        .withArgs(addrs[1].address, ZERO_ADDRESS, ethers.BigNumber.from('20000000'), ethers.BigNumber.from('2'), ethers.BigNumber.from('3'), ethers.BigNumber.from('3'))

      const voteState = await testingAsSigner1.isVoteDone(ethers.BigNumber.from('3'), ZERO_ADDRESS, ethers.BigNumber.from('20000000'), { from: addrs[1].address });
      expect(voteState.value.toString()).to.equal('0')

      const voteReceipt2 = testingAsSigner3.vote(ethers.BigNumber.from('3'), addrs[1].address, ZERO_ADDRESS, ethers.BigNumber.from('20000000'), { from: addrs[3].address });
      await expect(voteReceipt2).to.emit(obv, 'Voted')
        .withArgs(addrs[3].address, ZERO_ADDRESS, ZERO_ADDRESS, ethers.BigNumber.from('20000000'), ethers.BigNumber.from('3'))

      const voteStateFinal = await testingAsSigner1.isVoteDone(ethers.BigNumber.from('3'), ZERO_ADDRESS, ethers.BigNumber.from('20000000'), { from: addrs[1].address });
      ...
      await expect(setFeeReceipt).to.emit(obv, 'FeeUpdated')
        .withArgs(addrs[2].address, ethers.BigNumber.from('20000000'))
    });

    it("should create new withdraw vote", async () => {
        const receipt = testingAsSigner1.requestVote(ethers.BigNumber.from('0'), addrs[5].address, ethers.BigNumber.from('0'), { from: addrs[1].address });
        await expect(receipt).to.emit(obv, 'VoteRequested')
    ...
    it("should vote on new withdraw vote and finish", async () => {
        const createReceipt = testingAsSigner1.requestVote(ethers.BigNumber.from('0'), addrs[5].address, ethers.BigNumber.from('0'), { from: addrs[1].address });
        await expect(createReceipt).to.emit(obv, 'VoteRequested')
            .withArgs(addrs[1].address, addrs[5].address, ethers.BigNumber.from('0'), ethers.BigNumber.from('0'))

        const voteReceipt = testingAsSigner2.vote(ethers.BigNumber.from('0'), addrs[1].address, addrs[5].address, ethers.BigNumber.from('0'), { from: addrs[2].address });
        await expect(voteReceipt).to.emit(obv, 'Voted')
            .withArgs(addrs[2].address, addrs[5].address, addrs[5].address, ethers.BigNumber.from('0'), ethers.BigNumber.from('0'))

        const voteStateReceipt = testingAsSigner1.isVoteDone(ethers.BigNumber.from('0'), addrs[5].address, ethers.BigNumber.from('0'), { from: addrs[1].address });
        await expect(voteStateReceipt).to.emit(obv, 'VoteState')
            .withArgs(addrs[1].address, addrs[5].address, ethers.BigNumber.from('0'), ethers.BigNumber.from('2'), ethers.BigNumber.from('3'), ethers.BigNumber.from('0'))

        const voteState = await testingAsSigner1.isVoteDone(ethers.BigNumber.from('0'), addrs[5].address, ethers.BigNumber.from('0'), { from: addrs[1].address });
        expect(voteState.value.toString()).to.equal('0')

        const voteReceipt2 = testingAsSigner3.vote(ethers.BigNumber.from('0'), addrs[1].address, addrs[5].address, ethers.BigNumber.from('0'), { from: addrs[3].address });
        await expect(voteReceipt2).to.emit(obv, 'Voted')
            .withArgs(addrs[3].address, addrs[5].address, addrs[5].address, ethers.BigNumber.from('0'), ethers.BigNumber.from('0'))

        const voteStateReceipt2 = testingAsSigner1.isVoteDone(ethers.BigNumber.from('0'), addrs[5].address, ethers.BigNumber.from('0'), { from: addrs[1].address });
        await expect(voteStateReceipt2).to.emit(obv, 'VoteState')
            .withArgs(addrs[1].address, addrs[5].address, ethers.BigNumber.from('0'), ethers.BigNumber.from('3'), ethers.BigNumber.from('3'), ethers.BigNumber.from('0'))

        const voteStateFinal = await testingAsSigner1.isVoteDone(ethers.BigNumber.from('0'), addrs[5].address, ethers.BigNumber.from('0'), { from: addrs[1].address });
        expect(voteStateFinal.value.toString()).to.equal('0')
    });
    ...
    it("should create new vesting vault", async () => {
        ...
        await expect(receipt).to.emit(obv, 'VaultCreated')
            .withArgs(ethers.BigNumber.from('1'), gm_proxy.address, fee)
    });
    ...
    it("should create new vesting vault for deflationary", async () => {
        ...
        await expect(receipt).to.emit(obv, 'VaultCreated')
            .withArgs(ethers.BigNumber.from('2'), dft_proxy.address, fee)
    });
    ...
    it("should fail adding beneficiary, deflationary token", async () => {
        ...
        await expect(obv['addBeneficiary(address,address,uint256,uint256,uint256,uint256,uint8)'](dft_proxy.address, addrs[1].address, 100,
    ...
    it("should withdraw fee", async () => {
        ...
        await expect(setFeeReceipt).to.emit(obv, 'FeeWithdraw')
    ...
    it("should revert because vault exists already", async () => {
        await expect(obv.createVault(gm_proxy.address))
            .to.revertedWith("Vault exists already")
    });
    ...
    it("should add beneficiary, fixed", async () => {
        ...
        const block = await ethers.provider.getBlock("latest")
        const time = block.timestamp + 10;
        const receipt = obv['addBeneficiary(address,address,uint256,uint256,uint256,uint256,uint8)'](gm_proxy.address, addrs[1].address, 100,
            time, 1000 /* 1000 seconds */, 0, 0);
            await expect(receipt).to.emit(obv, 'AddedBeneficiary')
                .withArgs(ethers.BigNumber.from('1'), addrs[1].address, ethers.BigNumber.from('100'),
                    ethers.BigNumber.from(time), ethers.BigNumber.from(1000), ethers.BigNumber.from('0'))
    });
    ...
    it("should add beneficiary, linear", async () => {
        ...
        const block = await ethers.provider.getBlock("latest")
        const time = block.timestamp + 10;
        const receipt = obv['addBeneficiary(address,address,uint256,uint256,uint256,uint256,uint8)'](gm_proxy.address, addrs[2].address, 200,
            time, 1000 /* 1000 seconds */, 0, 1);
        await expect(receipt).to.emit(obv, 'AddedBeneficiary')
            .withArgs(ethers.BigNumber.from('1'), addrs[2].address, ethers.BigNumber.from('200'),
                ethers.BigNumber.from(time), ethers.BigNumber.from(1000), ethers.BigNumber.from('1'))
    });
    ...
    it("should match releaseable amount, linear", async () => {
        ...
        try {
            ...
            const block = await ethers.provider.getBlock("latest")
            const calculatedAmount = ethers.BigNumber.from(beneficiary.amount).mul(ethers.BigNumber.from(block.timestamp).sub(ethers.BigNumber.from(beneficiary.startTime))).div(ethers.BigNumber.from(beneficiary.duration));
    ...
    it("should add beneficiary with cliff, linear", async () => {
        ...
        const block = await ethers.provider.getBlock("latest")
        const time = block.timestamp + 10;
        const receipt = obv['addBeneficiary(address,address,uint256,uint256,uint256,uint256,uint8)'](gm_proxy.address, addrs[3].address, 200,
            time, 1000 /* 1000 seconds */, 100, 1);
        await expect(receipt).to.emit(obv, 'AddedBeneficiary')
            .withArgs(ethers.BigNumber.from('1'), addrs[3].address, ethers.BigNumber.from('200'),
                ethers.BigNumber.from(time), ethers.BigNumber.from(1000), ethers.BigNumber.from('1'))
    });
    ...
    it("should release vested amount, fixed", async () => {
        ...
        try {
            ...
            await expect(receipt).to.emit(obv, 'Fulfilled')
                .withArgs(ethers.BigNumber.from('1'), addrs[1].address, ethers.BigNumber.from(calculatedAmount), ethers.BigNumber.from(calculatedAmount))
    ...
    it("should release vested amount, linear", async () => {
        ...
        try {
            ...
            await expect(receipt).to.emit(obv, 'Release')
                .withArgs(ethers.BigNumber.from('1'), addrs[2].address, ethers.BigNumber.from(calculatedAmount), ethers.BigNumber.from(calculatedAmount))
            ...
            expect(ethers.BigNumber.from(releaseable).add(ethers.BigNumber.from(beneficiary2.released))).to.equal(beneficiary.amount)
            ...
            const receiptAgain = obv['addBeneficiary(address,address,uint256,uint256,uint256,uint256,uint8)'](gm_proxy.address, addrs[2].address, 100,
                time, 1000 /* 1000 seconds */, 100, 1);
            await expect(receiptAgain).to.emit(obv, 'AddedBeneficiary')
                .withArgs(ethers.BigNumber.from('1'), addrs[2].address, ethers.BigNumber.from('100'),
                    ethers.BigNumber.from(time), ethers.BigNumber.from(1000), ethers.BigNumber.from('1'))
    ...
    it("should release vested amount, with cliff, linear", async () => {
        ...
        try {
            ...
            await expect(receipt).to.emit(obv, 'Release')
                .withArgs(ethers.BigNumber.from('1'), addrs[3].address, ethers.BigNumber.from(calculatedAmount), ethers.BigNumber.from(calculatedAmount))
